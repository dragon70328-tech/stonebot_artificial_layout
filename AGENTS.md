...
- 从含“编”字的图层或 `NUMBERS` 图层（本系统编号检查文件约定）收集短文本，
  或从所有 TEXT/MTEXT 中按常见编号形态过滤
- 每个外轮廓取最近未用编号，默认搜索半径可随图面坐标偏移调整（如 5000）
- 无匹配的自动编号 `P-XXXX`

注意：编号检查 DXF（`OUTER/HOLES/NUMBERS` 图层）回读依赖 `NUMBERS` 图层约定；
缺失该约定时会静默丢掉编号匹配不到的零件（2026-08-30 水星 950→1258 修复）。
