...
- 水星完整桥切回归已降至约 `8.3` 分钟（几何加速后，1258 件：排板 393s + 后处理/校验约 100s），
  目标 `≤3` 分钟未达成；剩余热点在 touches/intersects 逐次调用开销与后处理阶段。
- `main.py` 已降至 `1166` 行；排板/后处理/报告编排已入 `src/workflow.py`，
  剩余为 CLI 参数、审图管道与交互模式。
...
- `scripts/check_output_cases.py` 需进一步明确慢案例的 skip/require-all 策略与 CI 超时；
  基线文件已过期（加拿大 25 张/21.4s、水星 950 件），刷新为当前结果。
- SaaS 组件目前主要是契约与内存实现，尚未接入本地 CLI、持久化队列、真实 Worker 与外部租户存储。
  CLI 已接入 WorkflowSession 记录层（2026-08-30）。
- 加拿大 3L 的 22 张最优结果仍依赖 `--budget 180`（201s），`--budget 60` 为 23 张；
  候选方向：LNS 结构化破坏算子（最低利用率整板/相邻零件簇）。
